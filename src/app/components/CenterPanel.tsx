...
const bankDetailsByPeriod: Record<
  string,
  Array<{
    name: string;
    bidRate: number;
...
export function CenterPanel({ onSendToTrade }: CenterPanelProps) {
  ...
  const [selectedQuoteForDetail, setSelectedQuoteForDetail] =
  ...
  const nonBankDetails = selectedQuoteForDetail
    ? getNonBankDetails(selectedQuoteForDetail)
    : [];
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  onClick={() => {
                    ...
                    link.setAttribute(
                      "download",
                      `非银机构报价明细_${selectedQuoteForDetail.period}_${new Date().toISOString().split("T")[0]}.csv`,
                    );
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-gray-100 border-b border-gray-300">
                    <th className="px-3 py-2 text-left font-bold text-gray-700 border-r border-gray-200">
                      机构名称
                    </th>
                    <th className="px-3 py-2 text-left font-bold text-gray-700 border-r border-gray-200">
                      类型
                ...
                <tbody>
                  {nonBankDetails
                    .filter((d) => d.institution.includes(nonBankSearch.trim()))
                    .map((detail, index) => (
                      <tr
                        key={detail.id}
                      ...
                      >
                        <td className="px-3 py-2 font-bold text-gray-900 border-r border-gray-200 whitespace-nowrap">
                          {detail.institution}
                        </td>
...
function QuoteSection({
  ...
  const handleDownload = () => {
    ...
    link.setAttribute(
      "download",
      `${title.replace(/[^a-zA-Z0-9\u4e00-\u9fa5]/g, "_")}_${new Date().toISOString().split("T")[0]}.csv`,
    );
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
  ...
  return (
    <div
      className={`bg-white rounded-lg border border-gray-300 shadow-sm ${highlight ? "ring-2 ring-blue-500/50" : ""} relative`}
    >
          ...
          <div className="bg-white border-2 border-blue-500 rounded-lg shadow-xl p-3 w-[280px]">
            ...
            <div className="mt-2 pt-2 border-t border-gray-200 text-xs text-gray-500">
              ...
              <span className="text-emerald-600 font-semibold">
                买{hoveredQuote.bidRate.toFixed(2)}%
              </span>
              <span> / </span>
              <span className="text-red-600 font-semibold">
                卖{hoveredQuote.askRate.toFixed(2)}%
              </span>
            </div>
          </div>
        </div>
...
function ExpandableBankQuoteSection({
  title,
  quotes,
  flashingIds,
  onSendToTrade,
}: ExpandableBankQuoteSectionProps) {
  ...
  const handleDownload = () => {
    ...
    quotes.forEach((q) => {
      ...
      (bankDetailsByPeriod[q.period] ?? []).forEach((b) => {
        rows.push([
          q.period,
          b.name,
          b.bidVolume.toString(),
          b.bidRate.toFixed(2),
          b.askRate.toFixed(2),
          b.askVolume.toString(),
        ]);
      });
    });
    const csv =
    ...
    link.setAttribute(
      "download",
      `大行指定机构报价_${new Date().toISOString().split("T")[0]}.csv`,
    );
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="bg-gray-100 border-b border-gray-300">
            ...
            <th className="min-w-[80px] px-3 py-2 text-left font-bold text-gray-700 border-r border-gray-200">
              期限
            </th>
            <th className="px-3 py-2 text-right font-bold text-gray-700 border-r border-gray-200">
              逆回购量(亿)
            ...
            return (
              <>
                ...
                {isExpanded &&
                  bankRows.map((bank) => (
                    <tr
                      key={`${quote.period}-${bank.name}`}
                      className="border-b border-gray-100 bg-blue-50/40 hover:bg-blue-50"
                    >
                      <td className="border-r border-gray-200"></td>
                      <td className="px-3 py-1.5 border-r border-gray-200">
                        <span className="pl-4 text-gray-700 font-medium">
                          {bank.name}
                        </span>
                      </td>
